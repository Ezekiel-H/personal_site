...
Enrollment <- readRDS("Election\\Data\\Enrollment.RDS")
votes <- readRDS("Election\\Data\\Votes.RDS")
...
hchart(melt, "column", hcaes(x = Year, y = value, group = variable)) %>% 
...
hchart(x1, "column", hcaes(x = Age, y =AgeEligable, group = Area)) %>%
...
hchart(x, "column", hcaes(x = District, y =`Electoral Majority`, fill = `Electorate Candidate`, group = Year, color = coloract)) %>% 
